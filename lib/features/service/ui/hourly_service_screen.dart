...
class HourlyServiceScreen extends StatefulWidget {
  ...
  const HourlyServiceScreen({key, required this.category}) : super(key: key);
...
class _HourlyCleaningScreen extends State<HourlyServiceScreen> {
  ...
  void initState() {
    ...
    _companyCubit.emitGetHourlyAllCompaniesStates();
  }
    ...
    return BlocProvider.value(
      ...
      child: BlocListener<CompanyCubit, CompanyState>(
        ...
        listenWhen: (previous, current) => 
            current is OrderHourlyAllCompaniesLoading ||
            current is OrderHourlyAllCompaniesSuccess ||
            current is OrderHourlyAllCompaniesError,


        listener: (context, state) {
          state.whenOrNull(orderHourlyAllCompaniesSuccess: (response) {
            context.pop();
            ...
            orderHourlyAllCompaniesError: (error) {
  ...
  bool isDetailComplete() {
    if (_currentStep == 0) {
      ...
      if (CacheHelper.getData(key: "selectIndexHourly") < 0) {
        return false;
      }
